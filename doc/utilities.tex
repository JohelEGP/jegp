...
\begin{itemdescr}
\pnum
Let \tcode{D} be \tcode{std::remove_cvref_t<DerivedRef>}
and \tcode{B} be \tcode{std::remove_cvref_t<Base>}.
...
\item
  \tcode{std::is_same_v<D, B>} is \tcode{false},
\item
  \tcode{std::derived_from<D, B>} is \tcode{true}, and
\item
